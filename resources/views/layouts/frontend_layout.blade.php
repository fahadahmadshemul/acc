        ...
        <div class="right-sidebar-content widget-area" id="sidebar-primary">
            ...
            <div class="visitor-counter-content" style="color: #000;" >
                @php
                    ...
                    //today
                    $date = date('d-m-Y');
                    $today = DB::table('visitors')->where('date', $date)->get()->count();
                    //yesterday
                    $yester_date = Carbon\Carbon::yesterday();
                    $yesterday = DB::table('visitors')->where('date', $yester_date)->get()->count();
                    //this week
                    $weekStartDate = $now->startOfWeek()->format('d-m-Y');
                    $weekEndDate = $now->endOfWeek()->format('d-m-Y');
                    $week = DB::table('visitors')->where('date', $weekStartDate)
                                ->orWhere('date', $weekEndDate)->get()->count();
                    //month 
                    $month = date('M');
                    $month_total = DB::table('visitors')->where('month', $month)->get()->count();
                    //total
                    $total = DB::table('visitors')->get()->count();
                    
                @endphp
                <p>Today: {{$today}}</p>
                <p>Yesterday: {{$yesterday}}</p>
                <p>This Week: {{$week}}</p>
                <p>This Month: {{$month_total}}</p>
                <p>Total: {{$total}}</p>
            </div>
            </aside>
...
<!--====== CATEGORY PART ENDS ======-->
    ...
    <script
    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDC3Ip9iVC0nIxC6V14CKLQ1HZNF_65qEQ&callback=initMap&libraries=&v=weekly"
    async
  ></script>
</body>
</html>
